...
export default function BookableNew() {
    ...
    const {mutate:createBookable, status,error} = useMutation(
        ...
        item =>createItem("http://localhost:3002/bookables",item),
        {
            // 비동기 함수 처리가 성공하면 실행되는 함수를 정의
            onSuccess: bookable =>{  // bookable : 방금 추가된 데이터
                queryClient.setQueryData(  // useQuery 가 관리하는 cache 를 수정
                    "bookables",  // key : bookables 수정
                    old=> [...(old || []),bookable]
                )
...
/*    function handleSubmit() {
        const result = createItem(`http://localhost:3002/bookables`, state)
        console.log('handleSubmit result', result)
        // navigate 를 방금 추가한 id 값으로 하려고 함
        // POST 결과로 리턴된 Promise 객체에서 id 값을 가져오기는 불편 -> useMutation hook 사용
